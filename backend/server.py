...
import os, logging, random, asyncio, json, uuid, sys
...
app.mount("/dashboard", StaticFiles(directory=ROOT_DIR / "dashboard", html=True), name="dashboard")
...
class DashboardTestRunRequest(BaseModel):
    kind: str = "pytest"
...
DASHBOARD_UPDATES = [
    {
        "title": "Core API",
        "desc": "Guest login, profile, avatars, daily rewards, streaks, spin wheel, and user state.",
        "endpoints": ["/api/", "/api/guest/login", "/api/user/{user_id}", "/api/avatars/list"],
    },
    {
        "title": "Game Flow",
        "desc": "Rooms, computer matches, claim-bingo, matchmaking, and live WebSocket play.",
        "endpoints": ["/api/rooms", "/api/computer-match", "/api/matchmaking/join", "/api/ws/room/{room_id}/{user_id}"],
    },
    {
        "title": "Progression",
        "desc": "Missions, battle pass, collection set, achievements, leaderboard, VIP, and cosmetics.",
        "endpoints": ["/api/missions/{user_id}", "/api/battle-pass/{user_id}", "/api/collection/{user_id}", "/api/achievements/{user_id}"],
    },
    {
        "title": "Economy",
        "desc": "Transactions, shop, Razorpay config / order / verify, and guild creation.",
        "endpoints": ["/api/transactions/{user_id}", "/api/shop/items", "/api/payments/razorpay/config", "/api/guilds/create"],
    },
]
...
async def dashboard_data():
    return {
        "title": "Bingo Blast API Dashboard",
        "subtitle": "A single place to explore the backend, test the major flows, and inspect live data shapes.",
        "sections": DASHBOARD_UPDATES,
        "collections": [
            {
                "name": "users",
                "purpose": "Profiles, wallet, progress, friends, cosmetics, VIP, push tokens, and match state.",
                "hot_fields": ["id", "username", "avatar", "bcoins", "wins", "matches", "streak", "bp_xp", "collection", "guild_id"],
            },
            {
                "name": "rooms",
                "purpose": "Public rooms, private rooms, matchmaking rooms, cards, readiness, and live round state.",
                "hot_fields": ["id", "code", "host_id", "players", "status", "called_numbers", "cards", "winner_id"],
            },
            {
                "name": "matches",
                "purpose": "Computer matches, bot flow, card state, powerups, bingo claims, and rewards.",
                "hot_fields": ["id", "user_id", "difficulty", "user_cards", "called_numbers", "status", "winner", "reward"],
            },
            {
                "name": "transactions",
                "purpose": "Economic audit trail for rewards, purchases, room entries, tournaments, VIP, and missions.",
                "hot_fields": ["id", "user_id", "type", "amount", "description", "payment_id", "created_at"],
            },
            {
                "name": "guilds",
                "purpose": "Guild creation, membership, code joins, leadership, and weekly / total points.",
                "hot_fields": ["id", "name", "tag", "code", "leader_id", "members", "weekly_points", "total_points"],
            },
        ],
        "endpoints": [
            {"group": "Core", "items": ["/api/", "/api/guest/login", "/api/user/{user_id}", "/api/avatars/list"]},
            {"group": "Progression", "items": ["/api/daily-reward/claim", "/api/streak/{user_id}", "/api/missions/{user_id}", "/api/battle-pass/{user_id}", "/api/collection/{user_id}", "/api/achievements/{user_id}"]},
            {"group": "Game Play", "items": ["/api/rooms", "/api/computer-match", "/api/computer-match/call", "/api/computer-match/dab", "/api/computer-match/use-powerup", "/api/computer-match/claim-bingo"]},
            {"group": "Live Multiplayer", "items": ["/api/rooms/{room_id}", "/api/rooms/{room_id}/join", "/api/ws/room/{room_id}/{user_id}", "/api/matchmaking/join", "/api/matchmaking/status/{entry_id}"]},
            {"group": "Economy", "items": ["/api/shop/items", "/api/payments/razorpay/config", "/api/payments/razorpay/create-order", "/api/payments/razorpay/verify", "/api/transactions/{user_id}"]},
            {"group": "Social", "items": ["/api/leaderboard", "/api/tournaments", "/api/tournament/register", "/api/friends/{user_id}", "/api/friends/add", "/api/vip/info/{user_id}", "/api/vip/activate"]},
            {"group": "Cosmetics & Guilds", "items": ["/api/cosmetics/{user_id}", "/api/cosmetics/equip", "/api/guilds/create", "/api/guilds/list", "/api/guilds/{guild_id}", "/api/guilds/join", "/api/guilds/leave"]},
            {"group": "Utility", "items": ["/api/push/register", "/api/dashboard", "/api/dashboard/data", "/api/docs"]},
        ],
        "major_updates": [
            "MongoDB Atlas persistence for users, rooms, matches, transactions, and guilds.",
            "FastAPI WebSocket multiplayer room support with live auto-caller game flow.",
            "Computer match flow with dab, powerups, and claim-bingo validation.",
            "Progression layers: daily streaks, missions, battle pass, collection, achievements, VIP.",
            "Commerce flows: shop catalog, Razorpay mock/live, and transaction history.",
        ],
    }


@api_router.get("/dashboard")
async def dashboard_api_alias():
    return {"redirect": "/dashboard/"}


@api_router.post("/dashboard/test-runs")
async def dashboard_start_test_run(req: DashboardTestRunRequest, request: Request):
    kind = req.kind.strip().lower()
    target = (req.target or "").strip()
    backend_url = str(request.base_url).rstrip("/")

    if kind == "pytest" and target:
        cmd = [sys.executable, "-m", "pytest", "-q", target]
    elif kind == "pytest":
        cmd = [sys.executable, "-m", "pytest", "-q"]
    elif kind == "engagement":
        cmd = [sys.executable, "-m", "pytest", "-q", "tests/test_engagement_phases.py"]
    elif kind == "extended":
        cmd = [sys.executable, "-m", "pytest", "-q", "tests/test_api_extended.py"]
    elif kind == "smoke":
        cmd = [sys.executable, "scripts/smoke_test_apis.py", "--base-url", backend_url, "--skip-ws"]
    else:
        raise HTTPException(400, "Unknown test kind")
    ...
    asyncio.create_task(_run_test_job(run_id, cmd))
    return {"ok": True, "run_id": run_id, "status": "queued", "command": cmd}


@api_router.get("/dashboard/test-runs")
async def dashboard_test_runs():
    runs = sorted(TEST_RUNS.values(), key=lambda r: r.get("created_at") or now_utc(), reverse=True)
    out = []
    for run in runs:
        out.append({
            **{k: v for k, v in run.items() if k not in {"lines", "tail"}},
            "created_at": iso(run["created_at"]) if isinstance(run.get("created_at"), datetime) else run.get("created_at"),
            "started_at": iso(run["started_at"]) if isinstance(run.get("started_at"), datetime) else run.get("started_at"),
            "finished_at": iso(run["finished_at"]) if isinstance(run.get("finished_at"), datetime) else run.get("finished_at"),
        })
    return out


@api_router.get("/dashboard/test-runs/{run_id}")
async def dashboard_test_run(run_id: str):
...
async def _run_test_job(run_id: str, cmd: List[str]):
    async with TEST_RUN_LOCK:
        run = TEST_RUNS.get(run_id)
        if not run:
            return
        run["status"] = "running"
